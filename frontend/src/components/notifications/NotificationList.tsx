...
const NotificationList = ({ autoRefresh = true, compact = false, refreshTrigger = 0, showAllTypes = true }) => {
  ...
  const [activeFilter, setActiveFilter] = useState<NotificationType>('all'); // 'all', 'mentions', 'comments', 'likes'
  ...
  useEffect(() => {
    ...
    if (autoRefresh) {
      interval = setInterval(fetchNotifications, 10000); // Every 3 seconds for more responsive updates
    }
